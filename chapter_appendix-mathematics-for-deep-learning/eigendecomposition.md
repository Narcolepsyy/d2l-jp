...
固有値は、線形代数を学ぶ際にしばしば最も有用な概念の一つです。
しかし、初心者にとってはその重要性を見落としやすいものでもあります。
以下では、固有分解を導入し、
それがなぜこれほど重要なのかを少しでも伝えたいと思います。
...
$\mathbf{A}\mathbf{v} = [2x, -y]^\top$ というベクトルが得られます。
これは直感的には、
ベクトルを $x$ 方向に2倍に引き伸ばし、
その後 $y$ 方向に反転させる、と解釈できます。

しかし、*ある種の*ベクトルでは、何かが変わらないまま残ります。
すなわち、$[1, 0]^\top$ は $[2, 0]^\top$ に写され、
$[0, 1]^\top$ は $[0, -1]^\top$ に写されます。
これらのベクトルは依然として同じ直線上にあり、
変化するのは行列がそれらをそれぞれ $2$ と $-1$ の倍率で伸縮することだけです。
このようなベクトルを*固有ベクトル*、
その伸縮の倍率を*固有値*と呼びます。
...
となるとき、$\mathbf{v}$ は $A$ の固有ベクトルであり、$\lambda$ は固有値であると言います。

## 固有値の求め方
それらをどのように求めるかを考えてみましょう。両辺から $\lambda \mathbf{v}$ を引き、
さらにベクトルをくくり出すと、上式は次と同値であることがわかります。

$$(\mathbf{A} - \lambda \mathbf{I})\mathbf{v} = 0.$$
:eqlabel:`eq_eigvalue_der`

:eqref:`eq_eigvalue_der` が成り立つためには、$(\mathbf{A} - \lambda \mathbf{I})$
がある方向をゼロまで押しつぶす必要があります。
したがってこれは可逆ではなく、ゆえに行列式はゼロです。
したがって、$\det(\mathbf{A}-\lambda \mathbf{I}) = 0$ となる $\lambda$ を求めることで、
*固有値*を見つけることができます。
固有値が求まれば、
$\mathbf{A}\mathbf{v} = \lambda \mathbf{v}$
を解くことで、対応する*固有ベクトル*を求められます。
...
と同値であることがわかります。
したがって、固有値は $4$ と $1$ の2つです。
対応するベクトルを求めるには、次を解く必要があります。
...
それぞれ、ベクトル $[1, -1]^\top$ と $[1, 2]^\top$ で解けます。

これを組み込みの `numpy.linalg.eig` ルーチンを使ってコードで確認できます。
...
行列 $\mathbf{A}$ の固有ベクトルを列にもつ行列とします。さらに
...
これは、行列の任意の正のべきについて、
固有分解では固有値を同じべきに上げるだけでよいことを示しています。
負のべきについても同様に示せるので、
行列を逆にしたいなら、各固有値を逆数にするだけでよいことになります。
...
可逆であることとゼロ固有値を持たないことは同じだとわかります。
...
すなわち全固有値の積になります。
これは直感的にも納得できます。$\mathbf{W}$ がどのような伸縮をしても、
$W^{-1}$ がそれを打ち消すので、最終的に起こる伸縮は
対角行列 $\boldsymbol{\Sigma}$ による掛け算だけです。
そして対角行列は、対角成分の積だけ体積を伸縮させます。

最後に、階数は行列の線形独立な列の最大数だったことを思い出してください。
固有分解を詳しく見ると、
階数は $\mathbf{A}$ のゼロでない固有値の数と同じであることがわかります。

例はいくらでも続けられますが、要点はおそらく明らかでしょう。
固有分解は多くの線形代数計算を簡単にし、
...
は、固有ベクトルをただ1つ、すなわち $(1, 0)^\top$ しか持ちません。
このような行列を扱うには、
ここで扱える範囲を超えるより高度な手法
（たとえばジョルダン標準形や特異値分解）が必要です。
そのため、しばしば固有ベクトルの完全な集合の存在を保証できる
行列に話を限定する必要があります。

最もよく現れるのは*対称行列*の族で、
これは $\mathbf{A} = \mathbf{A}^\top$ を満たす行列です。
...
## ガーシュゴリンの円定理
固有値は直感的に理解するのが難しいことがよくあります。
任意の行列を与えられても、計算せずに固有値について言えることはあまりありません。
しかし、対角成分が大きい場合に、よい近似を簡単に与えてくれる定理が一つあります。

$\mathbf{A} = (a_{ij})$ を任意の正方行列（$n\times n$）とします。
$r_i = \sum_{j \neq i} |a_{ij}|$ と定義します。
$\mathcal{D}_i$ を、中心が $a_{ii}$、半径が $r_i$ の複素平面上の円板とします。
すると、$\mathbf{A}$ のすべての固有値はどれか一つの $\mathcal{D}_i$ に含まれます。

これは少し分かりにくいので、例を見てみましょう。
次の行列を考えます。
...
$r_1 = 0.3$, $r_2 = 0.6$, $r_3 = 0.8$, $r_4 = 0.9$ です。
この行列は対称なので、固有値はすべて実数です。
したがって、固有値は次の範囲のいずれかに入ります。
...
このようにして、固有値は近似できます。
そして、対角成分が他のすべての要素より
かなり大きい場合には、その近似はかなり正確になります。

小さなことではありますが、固有分解のような複雑で繊細な話題では、
少しでも直感的な理解を得られるのは良いことです。

## 有用な応用: 反復写像の成長

固有ベクトルが原理的に何であるかを理解したので、
ニューラルネットワークの振る舞いの中心的な問題である
適切な重み初期化に対して、どのように深い理解を与えられるかを見てみましょう。

### 長期的な振る舞いとしての固有ベクトル

深層ニューラルネットワークの初期化に関する完全な数学的検討は
本書の範囲を超えますが、ここではそのおもちゃ版を見て、
固有値がこれらのモデルの働きをどう理解する助けになるかを見てみましょう。
ご存じのように、ニューラルネットワークは線形変換の層と
非線形演算を交互に挟むことで動作します。
ここでは簡単のため、非線形性はなく、
変換は単一の行列演算 $A$ の反復だと仮定します。
すると、モデルの出力は
...
そのような行列を一つ作ってみましょう。
具体的には、平均0、分散1のガウス分布に従う $5 \times 5$ 行列から始めます。
...
上の計算の最後の部分を見ると、
ランダムベクトルは `1.974459321485[...]` 倍に伸びていることがわかります。
末尾の部分は少し変動しますが、
伸長率は安定しています。

### 固有ベクトルとの関係

固有ベクトルと固有値は、何かがどれだけ伸びるかに対応することを見てきましたが、
それは特定のベクトルと特定の伸長についてでした。
では、$\mathbf{A}$ に対してそれらが何であるかを見てみましょう。
ここで少し注意があります。すべてを見るには、
複素数まで行く必要があることがわかります。
複素数は伸長と回転だと考えられます。
複素数のノルム
（実部と虚部の二乗和の平方根）を取ることで、
その伸長率を測れます。ついでに並べ替えましょう。
...
しかし、これを幾何学的に考えると、納得がいきます。
ランダムベクトルを考えましょう。
このランダムベクトルはあらゆる方向を少しずつ向いているので、
特に、$\mathbf{A}$ の最大固有値に対応する固有ベクトルと
少なくとも少しは同じ方向を向いています。
これは非常に重要なので、
*主固有値*および*主固有ベクトル*と呼ばれます。
$\mathbf{A}$ を適用すると、ランダムベクトルは
考えられるすべての方向に伸びますが、
最も強く伸びるのはこの主固有ベクトルに対応する方向です。
つまり、$A$ を適用した後では、
ランダムベクトルはより長くなり、
主固有ベクトルにより近い方向を向くようになります。
行列を何度も適用すると、
主固有ベクトルとの整列はどんどん近づき、
実用上は、ランダムベクトルは主固有ベクトルへと変換されたとみなせるようになります！
実際、このアルゴリズムは、
行列の最大固有値と固有ベクトルを求める
*べき乗法*の基礎になっています。詳細は、たとえば :cite:`Golub.Van-Loan.1996` を参照してください。
...
前と同様に連続するノルムの比も描けば、実際に安定していくことがわかります。
...
これで、私たちが望んでいたことがまさに起きているのがわかります！
主固有値で行列を正規化すると、
ランダムデータは以前のように爆発せず、
最終的にはある特定の値に落ち着きます。
これらを第一原理から導けるとよいのですが、
実際にその数学を深く見ると、
独立な平均0、分散1のガウス成分をもつ大きなランダム行列の最大固有値は、
平均するとおよそ $\sqrt{n}$、
この場合は $\sqrt{5} \approx 2.2$ になることがわかります。
