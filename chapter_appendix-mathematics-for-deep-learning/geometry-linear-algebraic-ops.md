...
それを用いてデータを変換する一般的な操作を表現できることを見ました。
線形代数は、深層学習や、より広く機械学習で行う多くの仕事を支える
重要な数学的柱の一つです。
:numref:`sec_linear-algebra` には現代の深層学習モデルの仕組みを伝えるのに十分な道具立てがありましたが、
この分野にはまだまだ多くの内容があります。
この節では、さらに深く掘り下げ、
線形代数の操作に関するいくつかの幾何学的解釈を示し、
固有値と固有ベクトルを含むいくつかの基本概念を導入します。

## ベクトルの幾何
まず、ベクトルの2つの一般的な幾何学的解釈、
すなわち空間内の点または方向としての解釈について議論する必要があります。
本質的には、ベクトルとは下の Python のリストのような数の並びです。
...
数学者はこれをたいてい *列* ベクトルまたは *行* ベクトルとして書きます。つまり、次のように表します。
...
これらはしばしば異なる解釈を持ち、
データ例は列ベクトル、
重み付き和を作るために使う重みは行ベクトルとして扱われます。
しかし、柔軟であることは有益です。
:numref:`sec_linear-algebra` で述べたように、
単一のベクトルの既定の向きは列ベクトルですが、
...
ベクトル $\mathbf{u}-\mathbf{v}$ は点 $\mathbf{v}$ から点 $\mathbf{u}$ へ移動する方向であることがわかります。
...
と書き、ベクトルの順序を入れ替えても同じ答えになることを強調します。

内積 :eqref:`eq_dot_def` には幾何学的な解釈もあります。それは2つのベクトルのなす角と密接に関係しています。 :numref:`fig_angle` に示す角を考えましょう。
...
ベクトル $\mathbf{w}$ は長さ $s$ で $x$ 軸となす角が $\theta$ です。
この2つのベクトルの内積を計算すると、
...
内積とノルムを組み合わせることで2つのベクトルのなす角がわかります。
この事実は一般の場合にも成り立ちます。ここでは導出はしませんが、
...
この結果は、計算のどこにも2次元という情報が現れないので、うれしい性質です。
実際、3次元でも300万次元でも問題なく使えます。

簡単な例として、ベクトルの組のなす角をどう計算するか見てみましょう。
...
今は使いませんが、角度が $\pi/2$
（同値に $90^{\circ}$）であるベクトルを *直交* していると言うことを知っておくと便利です。
上の式を見ると、これは $\theta = \pi/2$ のときに起こり、
これは $\cos(\theta) = 0$ と同じことです。
これが起こる唯一の方法は内積そのものが0である場合であり、
2つのベクトルは $\mathbf{v}\cdot\mathbf{w} = 0$ のとき、かつそのときに限り直交します。
これは、対象を幾何学的に理解するときに役立つ式です。

なぜ角度を計算することが有用なのか、という疑問はもっともです。
答えは、データに期待する不変性の種類にあります。
画像と、その複製で各ピクセル値が同じだが明るさが $10\%$ のものを考えましょう。
個々のピクセルの値は、一般に元の値からかなり離れています。
したがって、元の画像と暗い画像の距離を計算すると、その距離は大きくなりえます。
しかし、多くの機械学習アプリケーションでは、*内容* は同じです。---猫/犬分類器にとっては、依然として猫の画像です。
一方、角度を考えると、任意のベクトル $\mathbf{v}$ について、
$\mathbf{v}$ と $0.1\cdot\mathbf{v}$ のなす角は0であることはすぐにわかります。
これは、ベクトルをスケーリングしても向きは同じで、長さだけが変わることに対応しています。
角度は、暗い画像を同一とみなします。

このような例は至るところにあります。
テキストでは、同じことを言っている文書を2倍の長さで書いたとしても、
議論している話題が変わらないことを望むかもしれません。
...
反対方向を向くときに最小値 $-1$、
2つのベクトルが直交するときに値 $0$ を取ります。
高次元ベクトルの成分が平均 $0$ でランダムにサンプルされる場合、
そのコサインはほとんど常に0に近くなることに注意してください。


## 超平面

ベクトルを扱うことに加えて、線形代数を深く理解するために
もう一つ重要な対象があります。それは *超平面* であり、
これは線（2次元）や平面（3次元）の高次元への一般化です。
$d$ 次元ベクトル空間では、超平面は $d-1$ 次元を持ち、
空間を2つの半空間に分割します。

例から始めましょう。
列ベクトル $\mathbf{w}=[2,1]^\top$ があるとします。
「$\mathbf{w}\cdot\mathbf{v} = 1$ を満たす点 $\mathbf{v}$ はどれか？」を知りたいとします。
...
と言っているのと同じだとわかります。これは :numref:`fig_vector-project` に示されています。
これが成り立つ点の集合は、
ベクトル $\mathbf{w}$ に直角な直線です。
必要なら、この直線の方程式を求めて、
$2x + y = 1$、あるいは同値に $y = 1 - 2x$ であることを確認できます。

次に、$\mathbf{w}\cdot\mathbf{v} > 1$ や $\mathbf{w}\cdot\mathbf{v} < 1$ を満たす点の集合を考えると、
それぞれ射影が $1/\|\mathbf{w}\|$ より長い、または短い場合であることがわかります。
したがって、これら2つの不等式は直線の両側を定めます。
このようにして、空間を2つに切り分ける方法が得られます。
一方の側のすべての点は内積がしきい値未満で、もう一方の側はしきい値超過です。
これは :numref:`fig_space-division` に示されています。
...
与えられたベクトル $\mathbf{w}$ に直角な平面が得られます。
2つの不等式は、再び平面の両側を定めます。これは :numref:`fig_higher-division` に示されています。

![任意の次元の超平面は空間を2つに分ける。](../img/space-division-3d.svg)
:label:`fig_higher-division`

ここで可視化できる能力は尽きますが、
10次元でも、100次元でも、10億次元でもこれを行うことを妨げるものはありません。
これは機械学習モデルを考えるときによく起こります。
たとえば、:numref:`sec_softmax` のような線形分類モデルは、
異なる目標クラスを分離する超平面を見つける方法として理解できます。
この文脈では、そのような超平面はしばしば *決定平面* と呼ばれます。
深層学習による分類モデルの大半は、最後に softmax に入力される線形層で終わるので、
深層ニューラルネットワークの役割は、
目標クラスが超平面によってきれいに分離できるような非線形埋め込みを見つけることだと解釈できます。

手作りの例として、Fashion-MNIST データセット
（:numref:`sec_fashion_mnist` で見たもの）に含まれるTシャツとズボンの小さな画像を分類するために、
それらの平均の差ベクトルを使って決定平面を定め、
大まかなしきい値を目分量で決めるだけでも、かなり妥当なモデルを作れることに注意してください。まずデータを読み込み、平均を計算します。
...
これらの平均を詳しく調べると有益なので、どのように見えるかを描画してみましょう。この場合、平均は確かにぼやけたTシャツの画像に見えます。
...
完全に機械学習された解法では、しきい値をデータセットから学習します。この場合は、訓練データで良さそうに見えるしきい値を自分で目分量で決めました。
...
ベクトル、長さ、角度の幾何についてはしっかり理解できました。
しかし、まだ議論していない重要な対象が一つあります。
それは、行列で表される線形変換の幾何学的理解です。
2つの、場合によっては異なる高次元空間の間で、
行列がデータをどう変換するのかを完全に身につけるにはかなりの練習が必要であり、
この付録の範囲を超えています。
しかし、2次元から直感を積み上げ始めることはできます。
...
*2つの特定のベクトル*:
$[1,0]^\top$ と $[0,1]^\top$ に対する変換の仕方で表せることがわかります。
これは少し考える価値があります。
本質的には、無限個の問題
（任意の実数の組に何が起こるか）
を有限個の問題
...
に還元しているのです。
これらのベクトルは *基底* の一例であり、
空間内の任意のベクトルをこれらの *基底ベクトル* の重み付き和として書けます。
...
起こることは、行列積が格子を
せん断し、回転し、拡大縮小する一方で、
格子構造自体は :numref:`fig_grid-transform` に示すように保たれる、ということです。
...
これが、行列で表される線形変換について
身につけるべき最も重要な直感です。
行列は、空間のある部分だけを他の部分と異なるように歪めることはできません。
できるのは、空間上の元の座標を
せん断し、回転し、拡大縮小することだけです。

歪みが非常に大きくなることもあります。たとえば、行列
...
は2次元平面全体を1本の直線へと圧縮します。
このような変換を識別し扱うことは後の節の話題ですが、
幾何学的には、これは上で見た変換の種類とは本質的に異なることがわかります。
たとえば、行列 $\mathbf{A}$ の結果は元の格子へ「戻す」ことができます。
しかし、行列 $\mathbf{B}$ の結果は戻せません。
なぜなら、ベクトル $[1,2]^\top$ がどこから来たのかはわからないからです---それは
$[1,1]^\top$ だったのでしょうか、それとも $[0, -1]^\top$ だったのでしょうか？

この図は $2\times2$ 行列についてのものでしたが、
ここで学んだことを高次元に持ち込むことを妨げるものはありません。
...
これは平面全体を直線 $y = 2x$ 上に押しつぶします。
ここで疑問が生じます。行列そのものを見るだけで、
これを検出する方法はあるのでしょうか？
答えは、実際にある、です。
$\mathbf{B}$ の2つの列を $\mathbf{b}_1 = [2,4]^\top$ と $\mathbf{b}_2 = [-1, -2]^\top$ とします。
...
たとえば $\mathbf{b}_2$ だけを使ってこれら2つの列の任意の線形結合を書けることを意味します。なぜなら
...
これは、ある意味で一方の列が冗長であることを意味します。
空間内で一意な方向を定めていないからです。
これはそれほど驚くことではありません。
すでにこの行列が平面全体を1本の直線へ押しつぶすことを見たからです。
さらに、線形従属
...
その行列が空間をより低い次元へ圧縮していることの証拠です。
線形従属がなければ、ベクトルは *線形独立* であるといいます。
行列の列が線形独立なら、圧縮は起こらず、その操作は元に戻せます。

## 階数

一般の $n\times m$ 行列があるとき、
その行列がどの次元の空間へ写すのかを問うのは自然です。
*階数* として知られる概念がその答えになります。
前節では、線形従属が空間の低次元への圧縮の証拠であることを述べたので、
これを階数の概念の定義に使えます。
特に、行列 $\mathbf{A}$ の階数とは、
列の部分集合の中で線形独立な列の最大数です。たとえば、行列
...
は2つの列が線形従属なので $\textrm{rank}(B)=1$ です。
しかし、どちらか一方の列だけなら線形従属ではありません。
より難しい例として、
...
を考えると、たとえば最初の2列は線形独立ですが、
3列の4通りの組はどれも従属であることから、$\mathbf{C}$ の階数は2であると示せます。

この手順は、述べたとおり非常に非効率です。
与えられた行列の列のすべての部分集合を調べる必要があり、
したがって列数に対して指数時間になる可能性があります。
後で、行列の階数を計算するより計算効率のよい方法を見ますが、
今のところは、この概念がよく定義されており、その意味を理解するにはこれで十分です。

## 可逆性

上で、線形従属な列を持つ行列との掛け算は元に戻せない、
すなわち入力を常に復元できる逆操作は存在しないことを見ました。
しかし、フルランク行列
...
これは対角成分が1で、それ以外が0の行列です。
これを *単位* 行列と呼びます。
適用してもデータを変えない行列です。
行列 $\mathbf{A}$ が行ったことを打ち消す行列を見つけるには、
次を満たす行列 $\mathbf{A}^{-1}$ を見つけたいのです。
...
あるので、一般には解が存在すると期待できます。
実際、次の節では *行列式* と呼ばれる量を見ますが、
その値が0でなければ解を見つけられるという性質があります。
このような行列 $\mathbf{A}^{-1}$ を *逆行列* と呼びます。
...
これが本当にそうかは、上の式で与えられる逆行列を掛けると実際にうまくいくことを確認すればよいです。
...
### 数値的な問題
行列の逆行列は理論上は有用ですが、
実際には問題を解くのに行列の逆行列を *使う* ことは、たいてい望ましくないと言わなければなりません。
一般に、次のような線形方程式を解くには、
...
よりも、はるかに数値的に安定なアルゴリズムがあります。

小さな数で割ると数値的不安定性が生じうるのと同様に、
低階数に近い行列の逆行列を求めることも不安定になりえます。

さらに、行列 $\mathbf{A}$ が *疎*、
つまり非ゼロ値を少数しか含まないことはよくあります。
...
どのような場合に注意して進むべきかについての直感を与えたいと思います。
一般には、実務で逆行列を避けるのがよい経験則です。
...
強調された正方形を見てください。
これは辺が $(0, 1)$ と $(1, 0)$ で与えられる正方形であり、面積は1です。
...
では、この平行四辺形の面積を求めると5になることは、座標幾何の練習問題です。
...
があるとき、結果として得られる平行四辺形の面積は $ad-bc$ であることが計算によりわかります。
この面積を *行列式* と呼びます。
...
負の項については、これは一般に数学で採用される慣習の問題です。
行列が図形を反転させるなら、面積は符号反転すると考えます。
では、行列式が0のときに何がわかるか見てみましょう。
...
この行列の行列式を計算すると、
$2\cdot(-2 ) - 4\cdot(-1) = 0$ です。
上での理解に照らすと、これは納得できます。
$\mathbf{B}$ は元の図の正方形を
面積0の線分へと圧縮します。
そして実際、変換後に面積が0になるのは、
低次元空間へ圧縮される場合に限られます。
したがって、次の結果が成り立ちます。
行列 $A$ が可逆であるのは、行列式が0でないとき、かつそのときに限ります。

最後に、平面上に何らかの図形が描かれていると想像してください。
コンピュータ科学者のように考えると、
その図形を小さな正方形の集合に分解できるので、
図形の面積は本質的には分解に含まれる正方形の数にすぎません。
この図形を行列で変換すると、
各正方形は平行四辺形へ送られ、それぞれの面積は行列式で与えられます。
したがって、任意の図形について、行列式は
行列がその図形の面積をどれだけ拡大縮小するかを表す（符号付きの）数を与えることがわかります。

より大きな行列の行列式を計算するのは骨が折れますが、
直感は同じです。
行列式は、$n\times n$ 行列が $n$ 次元体積をどれだけ拡大縮小するかを与える係数であり続けます。

## テンソルと一般的な線形代数操作

:numref:`sec_linear-algebra` ではテンソルの概念を導入しました。
この節では、テンソル縮約
（行列積に相当するテンソル版）をさらに深く掘り下げ、
それが多くの行列・ベクトル操作を統一的に捉える視点を与えることを見ます。

行列やベクトルでは、それらを掛けてデータを変換する方法を知っていました。
テンソルが私たちにとって有用であるためには、同様の定義が必要です。
行列積を考えてみましょう。
...
このパターンはテンソルにも繰り返し使えます。
テンソルでは、何について和を取るかを一意に普遍的に選べるわけではないので、
どの添字について和を取るのかを正確に指定する必要があります。
たとえば、
...
このような変換を *テンソル縮約* と呼びます。
これは、行列積だけよりもはるかに柔軟な変換の族を表せます。

よく使われる記法上の簡略化として、
和を取る添字は式中に2回以上現れる添字ちょうどであることに注目できます。
そのため、人々はしばしば *アインシュタイン記法* を用い、
繰り返し現れる添字に対して和が暗黙に取られるようにします。
これにより、次の簡潔な表現が得られます。
...
このようにして、無数の特殊な記法を短いテンソル表現に置き換えられます。

### コードで表す
テンソルはコード中でも柔軟に操作できます。
:numref:`sec_linear-algebra` で見たように、
以下のようにテンソルを作成できます。
...
これは非常に柔軟な記法です。
たとえば、伝統的には次のように書くものを計算したいとします。

$$
c_{kl} = \sum_{ij} \mathbf{b}_{ijk}\mathbf{a}_{il}v_j.
$$

これはアインシュタイン総和を使って次のように実装できます。
...
この記法は人間にとって読みやすく効率的ですが、
何らかの理由でテンソル縮約をプログラム的に生成する必要がある場合には、
ややかさばります。
このため、`einsum` には各テンソルに整数の添字を与える別の記法もあります。
たとえば、同じテンソル縮約は次のようにも書けます。
...
どちらの記法でも、コード中でテンソル縮約を簡潔かつ効率的に表現できます。
