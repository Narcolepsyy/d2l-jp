...
:eqref:`eq_pmi_def` は、独立な確率的結果に対して期待される場合と比べて、特定の結果の組 $x$ と $y$ がどれだけ起こりやすいか、あるいは起こりにくいかを測っていると考えられる。大きく正なら、これら2つの特定の結果は、ランダムな偶然と比べてはるかに高い頻度で起こります（*注*：分母は $p_X(x) p_Y(y)$ であり、2つの結果が独立である場合の確率です）。一方、大きく負なら、ランダムな偶然から期待されるよりもはるかに低い頻度で2つの結果が起こることを表す。
