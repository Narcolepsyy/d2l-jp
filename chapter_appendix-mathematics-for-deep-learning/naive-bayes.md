...
ひどく失敗した！その理由を調べるために、画素ごとの確率を見てみよう。通常 $0.001$ から $1$ の間の数である。それらを784個掛け合わせている。ここで重要なのは、これらの数をコンピュータ上で計算しているため、指数部の範囲が有限であるということである。起こるのは *数値アンダーフロー* で、小さな数をすべて掛け合わせるとさらに小さくなり、最終的には0に丸め込まれてしまいる。 :numref:`sec_maximum_likelihood` で理論的な問題として議論したが、ここではその現象が実際に明確に見える。
