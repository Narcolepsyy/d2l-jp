...
:numref:`sec_prob` では、離散確率変数の扱い方の基礎を見た。ここでいう離散確率変数とは、有限個の取りうる値、または整数値をとる確率変数のことである。この節では、任意の実数値をとりうる *連続確率変数* の理論を展開する。
...
しかし、よく見ると、私たちの問いと一致していない。私たちが知りたかったのは「ちょうど」当たる確率であって、これらのビンはたとえば $1.5\textrm{cm}$ から $2.5\textrm{cm}$ の間に入ったものをすべて含んでいるからである。

それでもあきらめず、さらに進む。今度は $1.9\textrm{cm}$、$2.0\textrm{cm}$、$2.1\textrm{cm}$ のように、もっと精密に測る。すると、$100$ 本のうち $3$ 本が $2.0\textrm{cm}$ のビンに入ったとしよう。そこで確率は $3\%$ だと結論づける。
...
これをさらに一歩進めよう。ここまでずっと点 $2$ について考えてきたが、他の点については考えていなかった。そこでも本質的には何も違いないが、値 $p$ はおそらく異なる。少なくとも、ダーツ投げでは $20\textrm{cm}$ より $2\textrm{cm}$ のように中心に近い点に当たりやすいと期待したいところである。したがって、値 $p$ は固定ではなく、点 $x$ に依存すべきである。すると、次を期待すべきだとわかる。

$$P(\textrm{distance is in an}\; \epsilon \textrm{-sized interval around}\; x ) \approx \epsilon \cdot p(x).$$
:eqlabel:`eq_pdf_deriv`

実際、:eqref:`eq_pdf_deriv` は *確率密度関数* を正確に定義している。、ある点の近くに当たる相対的な確率を表す関数 $p(x)$ である。このような関数がどのような形をしているかを可視化してみよう。
...
$$
 ...
 :numref:`sec_integral_calculus` で議論した積分の近似にほかならない。したがって、
...
前節では p.d.f. の概念を見た。実際には、連続確率変数を扱う際によく使われる方法であるが、1つ大きな落とし穴がある。それは、p.d.f. の値そのものは確率ではなく、確率を得るために積分しなければならない関数だということである。密度が $10$ より大きくても、それが長さ $1/10$ を超える区間にわたって大きくなければ問題ない。直感に反することがあるため、人々はしばしば *累積分布関数*、すなわち c.d.f. でも考える。こちらは *確率* である。
...
4つ目について、$X$ が離散的、たとえば $0$ と $1$ をそれぞれ確率 $1/2$ でとる場合には、成り立たないことに注意する。その場合、
...
*平均* は確率変数の平均値を表す。離散確率変数 $X$ が値 $x_i$ を確率 $p_i$ でとるなら、平均は重み付き平均で与えられる。すなわち、値にその値をとる確率を掛けて足し合わせる。
...
となる。したがって、平均は $a$ である。、確率変数を中心化した位置が $a$ であるという直感と一致している。
...
そこで、確率変数の *分散* を考える。、確率変数が平均からどれだけずれるかを定量的に測るものである。式 $X - \mu_X$ を考えよう。確率変数の平均からの偏差である。この値は正にも負にもなりうるので、偏差の大きさを測るために正にする工夫が必要である。

試してみる自然な方法は $\left|X-\mu_X\right|$ を見ることである。実際、 *平均絶対偏差* と呼ばれる有用な量につながりるが、数学や統計の他分野とのつながりのため、しばしば別の方法が使われる。
...
例として、$X$ が確率 $p$ で $a-2$、確率 $p$ で $a+2$、確率 $1-2p$ で $a$ をとる確率変数を考えよう。この場合 $\mu_X = a$ なので、計算すべきなのは $E\left[X^2\right]$ だけである。すぐに求められる。
...
この結果もまた納得できる。$p$ の最大値は $1/2$ であり、 $a-2$ か $a+2$ をコイン投げで選ぶ場合に対応する。このとき分散が $4$ になるのは、$a-2$ と $a+2$ のどちらも平均から $2$ だけ離れており、$2^2 = 4$ だからである。逆に、$p=0$ ならこの確率変数は常に $0$ をとるので、分散はまったくない。
...
これらの値を解釈する際には、少し引っかかる点がある。特に、単位を追跡しながらこの計算がどうなるかを考えてみよう。ウェブページ上で製品に付けられた星評価を扱っているとする。このとき $a$、$a-2$、$a+2$ はすべて星の単位で測られる。同様に、平均 $\mu_X$ も（重み付き平均なので）星の単位である。しかし分散に進むと、すぐに問題にぶつかる。$(X-\mu_X)^2$ を見る必要があり、 *二乗された星* の単位になるからである。つまり、分散そのものは元の測定値と比較しにくいのである。解釈可能にするには、元の単位に戻る必要がある。
...
ここで、「標準偏差は元の確率変数と同じ単位を持つのなら、その確率変数について何か描けるものを表しているのだろうか？」と問うのは自然である。答えは大いにイエスである。平均が確率変数の典型的な位置を示したのと同様に、標準偏差はその確率変数の典型的な変動範囲を与える。チェビシェフの不等式として知られるものによって厳密にできる。
...
となる。つまり、どんな $p$ に対しても、この確率変数は $75\%$ の確率でこの区間に入る。ここで、$p \rightarrow 0$ とすると、この区間は単一点 $a$ に収束する。しかし、確率変数は $a-2, a, a+2$ の値しかとらないので、最終的には $a-2$ と $a+2$ が区間の外に出ることは確実である。問題は、それがどの $p$ で起こるかである。そこで、$a+4\sqrt{2p} = a+2$ となる $p$ を解くと、$p=1/8$ であり、分布からの標本の $1/4$ 以下しか区間の外に出ないという主張に反しないまま、それが起こりうる最初の $p$ である（左に $1/8$、右に $1/8$）。
...
2つ目は、$p = 1/8$ のとき、区間がちょうど2点に接することを示する。、不等式を真に保ちながらこれ以上小さい区間を取れないので、不等式が *鋭い* ことを示している。
...
3つ目は、$p < 1/8$ のとき、区間が中心だけを含むことを示する。不等式を破るものではない。なぜなら、区間の外に出る確率が $1/4$ を超えないことだけを保証すればよいからである。したがって、$p < 1/8$ になれば、$a-2$ と $a+2$ の2点は捨ててよいのである。
...
と書ける。ここで $p_X$ は $X$ の密度である。 $xp_X(x)$ の積分の近似なので、
...
平均、分散、標準偏差について上で述べたことは、この場合にもすべて当てはまる。たとえば、密度
...
警告として、もう1つの例、*コーシー分布* を見てみよう。 p.d.f. が
...
何がまずいのかを見るために、これの分散を計算してみよう。 :eqref:`eq_var_def` を使って
...
となる。右辺の積分は対数の定義なので、本質的には $\log(\infty) = \infty$ であり、平均値も定義できない。
...
となるはずである。ここで $p(x, y)$ はある関数である。 $X$ と $Y$ の同時密度と呼ばれる。1変数の場合と同様に、次の性質が成り立ちる。
...
ここには $Y$ の記述がないが、与えられているのが $p _{X, Y}$ だけなら、何らかの形で $Y$ を含める必要がある。まず、次と同じだとわかる。
...
、 :numref:`fig_marginal` に示すように、密度の値を一列に並んだ正方形に沿って足し合わせることを意味する。実際、両辺から $\epsilon$ の因子を1つ打ち消し、右辺の和が $y$ に関する積分であることを認識すると、
...
複数の確率変数を扱うとき、知っておくと役立つもう1つの要約統計量が *共分散* である。、2つの確率変数がどの程度一緒に変動するかを測る。
...
ここで $p$ は $[0,1]$ の範囲で選べるパラメータである。$p=1$ なら、両者は常に同時に最小値または最大値をとり、$p=0$ なら、同時に反転した値をとることが保証される（片方が大きいとき他方は小さく、その逆も同様である）。$p=1/2$ なら4つの可能性はすべて同確率で、互いに関係がないはずである。共分散を計算してみよう。まず、$\mu_X = 2$、$\mu_Y = 1$ なので、:eqref:`eq_cov_def` を使って
...
共分散についての簡単な注意として、線形関係しか測らないという点がある。$Y$ が $\{-2, -1, 0, 1, 2\}$ から等確率で選ばれ、$X = Y^2$ のようなより複雑な関係は見逃されることがある。実際、簡単な計算により、一方が他方の決定論的関数であるにもかかわらず、これらの確率変数の共分散は0になる。
...
と定義すると、単位のない値になる。少し数学を使えば、この数は $-1$ から $1$ の間にあり、$1$ は最大の正の相関、$-1$ は最大の負の相関を意味することが示せる。
...
となる。 $-1$ から $1$ の範囲をとり、$1$ が最も相関が強く、$-1$ が最も相関が弱いという期待通りの振る舞いをする。
...
となる。したがって、$a > 0$ なら相関は $+1$、$a < 0$ なら $-1$ である。、相関が2つの確率変数の関係の程度と方向を測るのであって、変動のスケールを測るのではないことを示している。
...
最後に、いくつかの式が見覚えがあるように感じるかもしれない。実際、$\mu_X = \mu_Y = 0$ と仮定してすべて展開すると、

$$
\rho(X, Y) = \frac{\sum_{i, j} x_iy_ip_{ij}}{\sqrt{\sum_{i, j}x_i^2 p_{ij}}\sqrt{\sum_{i, j}y_j^2 p_{ij}}}.
$$

となる。、項の積の和を、項の和の平方根で割った形である。、確率 $p_{ij}$ で各座標が重み付けされた2つのベクトル $\mathbf{v}, \mathbf{w}$ のなす角の余弦の公式そのものである。
