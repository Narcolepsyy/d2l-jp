...
# ユーティリティ（Utility）関数とクラスとは
:label:`sec_utils`

**ユーティリティ（Utility）とは**、プログラムのさまざまな場所で共通して使用される、便利で汎用的な関数やクラス群のことである。本章では、進捗の可視化、ハイパーパラメータの保存、データセットのダウンロードなど、深層学習の実装を簡略化するための便利なツールをまとめている。
