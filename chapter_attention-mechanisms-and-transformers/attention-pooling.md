...
1. すべての $\mathbf{x}$ が単位球面上にある、すなわちすべて $\|\mathbf{x}\| = 1$ を満たすと仮定する。指数関数内の $\|\mathbf{x} - \mathbf{x}_i\|^2$ の項を簡単化できるか？ ヒント: 後で、ドット積アテンションと非常に密接に関係していることが分かる。 
1. :citet:`mack1982weak` が Nadaraya--Watson 推定の整合性を証明したことを思い出してほしい。データが増えるにつれて、アテンション機構のスケールをどのくらいの速さで小さくすべきだろうか。 答えの直感も述べよ。データの次元に依存するか？ どのように依存するか？
