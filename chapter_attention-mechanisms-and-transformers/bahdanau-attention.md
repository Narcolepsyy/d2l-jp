...
重要な考え方は、状態、すなわちソース文を要約するコンテキスト変数 $\mathbf{c}$ を固定したままにするのではなく、元のテキスト（エンコーダの隠れ状態 $\mathbf{h}_{t}$）と、すでに生成されたテキスト（デコーダの隠れ状態 $\mathbf{s}_{t'-1}$）の両方の関数として動的に更新することである。これにより $\mathbf{c}_{t'}$ が得られ、任意のデコード時刻 $t'$ の後に更新される。入力系列の長さが $T$ だとしよう。この場合、コンテキスト変数は注意プーリングの出力である：
...
、各デコードステップで入力系列の異なる部分が注意プーリングで選択的に集約されていることを示している。
...
トークンを予測するとき、入力トークンのすべてが関連するとは限らない場合、バーダナウ注意機構を備えた RNN エンコーダ--デコーダは入力系列の異なる部分を選択的に集約する。、状態（コンテキスト変数）を加法的注意プーリングの出力として扱うことで実現される。
