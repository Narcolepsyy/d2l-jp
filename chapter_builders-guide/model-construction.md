...
しかも、その組み合わせはしばしば再帰的に行える。 :numref:`fig_blocks` に示されている。任意の複雑さをもつモジュールを必要に応じて生成するコードを定義することで、
驚くほど簡潔なコードで
...
1. 出力の入力に関する勾配を計算する。逆伝播メソッドを通じてアクセスできる。通常、自動的に行われる。
...
1. 2つのモジュール、たとえば `net1` と `net2` を引数に取り、順伝播で両方のネットワークの連結出力を返すモジュールを実装せよ。*並列モジュール*とも呼ばれる。
