...
まずネットワークを初期化して
パラメータ辞書を生成する必要があります。
任意の層のパラメータには、この辞書のキーを通してアクセスできます。
:end_tab:

次のようにして、2番目の全結合層のパラメータを調べられます。
...
それぞれその層の重みとバイアスに対応しています。

### [**対象を絞ったパラメータ**]

各パラメータは
パラメータクラスのインスタンスとして表現されることに注意してください。
パラメータを使って何か有用なことをするには、
まずその内部の数値を取り出す必要があります。
その方法はいくつかあります。
より簡単なものもあれば、より一般的なものもあります。
次のコードは、2番目のニューラルネットワーク層から
バイアスを取り出します。これはパラメータクラスのインスタンスを返し、
さらにそのパラメータの値にアクセスします。
...
複雑なオブジェクトです。
そのため、値を明示的に要求する必要があります。

値に加えて、各パラメータからは勾配にもアクセスできます。このネットワークではまだ逆伝播を呼び出していないため、初期状態のままです。
:end_tab:

:begin_tab:`jax`
他のフレームワークとは異なり、JAX はニューラルネットワークのパラメータに対する勾配を追跡しません。その代わり、パラメータとネットワークは分離されています。
これにより、ユーザーは計算を Python 関数として表現し、同じ目的のために `grad` 変換を使うことができます。
:end_tab:
...
しばしば、複数の層にまたがってパラメータを共有したいことがあります。
それをエレガントに行う方法を見てみましょう。
以下では全結合層を1つ用意し、
そのパラメータを使って別の層のパラメータを設定します。
ここでは、パラメータにアクセスする前に
順伝播 `net(X)` を実行する必要があります。
...
結び付けられていることを示しています。
それらは単に等しいだけではなく、
まったく同じテンソルとして表現されています。
したがって、どちらか一方のパラメータを変更すると、
もう一方も変化します。

:begin_tab:`mxnet, pytorch, tensorflow`
パラメータが共有されているとき、
勾配はどうなるのか疑問に思うかもしれません。
