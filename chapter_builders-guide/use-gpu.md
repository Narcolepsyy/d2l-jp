...
大きな機会をもたらすが、同時に、そのような性能に対する大きな需要があったことも示している。
...
多くのデスクトップコンピュータにとっては贅沢かもしれないが、たとえばAWS EC2のマルチGPUインスタンスを使えば、クラウド上では簡単に利用できる。
...
、MXNetの計算がすべてのCPUコアを使おうとすることを意味する。
...
、PyTorchの計算がすべてのCPUコアを使おうとすることを意味する。
...
、そのような操作が、一方のデバイスが他方を待たなければ次のことができない場合にブロックされうるからである。
、電話で事前注文しておいて、あなたが来たときにはもうできあがっているコーヒーを受け取るのではなく、列に並んでコーヒーを注文するのに少し似ている。
