...
MXNet にすべてのバックエンド計算を返却前に完了させるよう強制すると、以前何が起きていたかが分かる。つまり、フロントエンドが Python に制御を返している間に、計算はバックエンドで実行されているのである。
:end_tab:
...
PyTorch にすべての計算を返却前に完了させるよう強制すると、以前何が起きていたかが分かる。つまり、フロントエンドが Python に制御を返している間に、計算はバックエンドで実行されているのである。
:end_tab:
...
バックエンドは独自のスレッドを管理し、キューに入ったタスクを継続的に収集して実行する。これが機能するためには、バックエンドが計算グラフ内のさまざまなステップ間の依存関係を追跡できなければならないことに注意しようう。したがって、互いに依存する操作を並列化することはできない。
:end_tab:
...
さまざまなステップ間の依存関係を追跡できなければならないことに注意しようう。
...
これら 3 段階の所要時間をそれぞれ $t_1, t_2, t_3$ とする。非同期プログラミングを使わない場合、10000 回の計算にかかる総時間はおおよそ $10000 (t_1+ t_2 + t_3)$ である。非同期プログラミングを使うと、10000 回の計算にかかる総時間は $t_1 + 10000 t_2 + t_3$ にまで削減できる（$10000 t_2 > 9999t_1$ と仮定した場合）。、各ループごとにフロントエンドがバックエンドから計算結果が返るのを待つ必要がないためである。
:end_tab:
...
:begin_tab:`mxnet`
* MXNet のメモリ管理領域から Python への変換は、特定の変数が準備できるまでバックエンドを待機させることに注意しようう。`print`、`asnumpy`、`item` などの関数はすべてこの影響を受ける。望ましい場合もあるが、同期を不用意に使うと性能を台無しにすることがある。
:end_tab:
