...
その場合、それらのカテゴリだけでなく、画像内での具体的な位置も知りたいのである。
...
バウンディングボックスは長方形であり、長方形の左上隅の $x$ 座標と $y$ 座標、および右下隅の同様の座標によって決まる。 
...
描画する前に、補助関数 `bbox_to_rect` を定義する。バウンディングボックスを `matplotlib` パッケージのバウンディングボックス形式で表す。
...
1. `box_corner_to_center` と `box_center_to_corner` の入力引数 `boxes` の最内次元が常に 4 であるのはなぜであるか？
