# ファインチューニング（Fine-tuning）とは：転移学習の実践
:label:`sec_fine_tuning`

**ファインチューニング（Fine-tuning）とは**、大規模なデータセット（ImageNetなど）で事前に学習されたモデルの重みを初期値として利用し、ターゲットとなる特定のタスクや比較的小さなデータセットに合わせてモデルを微調整する、転移学習の代表的な手法である。
...
しかし、データの収集とラベル付けには多くの時間と費用がかかる。
...
1. 新しいニューラルネットワークモデル、すなわち *ターゲットモデル* を作成する。出力層を除いて、ソースモデル上のすべてのモデル設計とそのパラメータをコピーする。これらのモデルパラメータにはソースデータセットで学習した知識が含まれており、その知識はターゲットデータセットにも適用できると仮定する。また、ソースモデルの出力層はソースデータセットのラベルと密接に関係していると仮定するため、ターゲットモデルでは使用しない。
...
[**ここで使用するホットドッグデータセットは、オンライン画像から取得したものである**]。
このデータセットは、
...
最初の 8 個の正例と最後の 8 個の負例画像を以下に示す。見てわかるように、[**画像のサイズとアスペクト比はさまざまである**]。
...
次に、ターゲットモデルとして新しいニューラルネットワークを構築する。事前学習済みのソースモデルと同じ方法で定義されるが、
最終層の出力数だけが
