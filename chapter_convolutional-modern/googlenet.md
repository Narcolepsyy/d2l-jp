...
GoogLeNet における重要な貢献は、ネットワーク本体の設計であった。畳み込みカーネルの選択という問題を独創的な方法で解決した。$1 \times 1$ から $11 \times 11$ までのどの畳み込みが最適かを特定しようとした他の研究とは異なり、GoogLeNet は単にマルチブランチ畳み込みを *連結* した。以下では、GoogLeNet のやや簡略化した版を紹介する。元の設計には、ネットワークの複数層に適用される中間損失関数を通じて学習を安定化させるためのいくつかの工夫が含まれていた。より優れた学習アルゴリズムが利用可能になった現在では、もはや必要ない。
...
ここから GoogLeNet を少しずつ実装していこう。まず stem から始める。最初のモジュールは 64 チャネルの $7\times 7$ 畳み込み層を使う。
...
GoogLeNet モデルは計算量の大きいモデルである。選択されたチャネル数、次元削減前のブロック数、チャネル間での容量配分の比率など、比較的恣意的なハイパーパラメータが非常に多いことに注意しようう。その多くは、GoogLeNet が導入された当時、ネットワーク定義や設計探索のための自動化ツールがまだ利用できなかったことに起因する。たとえば現在では、優れた深層学習フレームワークが入力テンソルの次元を自動的に推論できるのは当然だと考える。当時は、そのような設定の多くを実験者が明示的に指定しなければならず、活発な実験の速度をしばしば低下させていた。さらに、自動探索に必要なツールもまだ発展途上で、初期の実験は主として高コストな総当たり探索、遺伝的アルゴリズム、その他類似の戦略に頼っていた。
...
GoogLeNet の重要な特徴は、先行モデルよりも計算コストが*低い*にもかかわらず、精度が向上していることである。、ネットワークを評価するコストと誤差削減とのトレードオフを意識した、より慎重なネットワーク設計の始まりを示している。また、当時は完全に手作業ではあったものの、ブロック単位でネットワーク設計のハイパーパラメータを試行する時代の始まりでもあった。ネットワーク構造の探索戦略を議論する :numref:`sec_cnn-design` で、この話題に再び触れる。
...
1. AlexNet、VGG、NiN、GoogLeNet のモデルパラメータサイズを比較しよう。後者2つのネットワークアーキテクチャは、どのようにしてモデルパラメータサイズを大幅に削減しているのだろうか？
1. GoogLeNet と AlexNet に必要な計算量を比較しよう。、たとえばメモリ容量、メモリ帯域幅、キャッシュサイズ、計算量、特殊演算の利点といった観点で、アクセラレータチップの設計にどのような影響を与えるだろうか？
