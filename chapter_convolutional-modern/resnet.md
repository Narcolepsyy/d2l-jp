...
# 残差ネットワーク（ResNet）とResNeXtとは
:label:`sec_resnet`

**残差ネットワーク（ResNet: Residual Network）とは**、非常に深いニューラルネットワークの学習を可能にするために、入力データをそのまま後の層にバイパスさせる「残差接続（ショートカット接続）」を導入した画期的なCNNアーキテクチャである。
...
この場合は `use_1x1conv=True` として $1 \times 1$ 畳み込みを使う。各 ResNet ブロックの先頭で、`strides=2` によって空間次元を削減する際に便利である。
...
![ResNeXt ブロック。$g$ 個のグループを持つ grouped convolution は、密な畳み込みよりも $g$ 倍高速である。中間チャネル数 $\mathit{b}$ が $\mathit{c}$ より小さいとき、ボトルネック残差ブロックである。](../img/resnext-block.svg)
...
ここまで議論してきた設計に共通する特徴は、ネットワーク設計がかなり手作業に依存しており、主として設計者の独自の設計に頼って「正しい」ネットワークハイパーパラメータを見つける点である。明らかに実現可能ではあるが、人手の時間という観点では非常にコストが高く、結果が何らかの意味で最適である保証もない。 :numref:`sec_cnn-design` では、より自動化された方法で高品質なネットワークを得るためのいくつかの戦略を議論する。特に、RegNetX/Y モデルにつながった *ネットワーク設計空間* の概念を概観する :cite:`Radosavovic.Kosaraju.Girshick.ea.2020`。
