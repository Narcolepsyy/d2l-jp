...
最初は、[**$1 \times 1$ 畳み込み**]、すなわち $k_\textrm{h} = k_\textrm{w} = 1$ は、あまり意味がないように見える。そもそも、畳み込みは隣接するピクセルを相関させるものである。$1 \times 1$ 畳み込みは明らかにそうではない。それにもかかわらず、複雑な深層ネットワークの設計にしばしば含まれる人気のある演算である :cite:`Lin.Chen.Yan.2013,Szegedy.Ioffe.Vanhoucke.ea.2017`。実際に何をしているのかを、もう少し詳しく見てみよう。

最小のウィンドウを使うため、$1\times 1$ 畳み込みは、高さ方向と幅方向の隣接要素どうしの相互作用から成るパターンを認識するという、より大きな畳み込み層の能力を失う。$1\times 1$ 畳み込みで行われる計算は、チャネル次元上だけである。

:numref:`fig_conv_1x1` は、3 つの入力チャネルと 2 つの出力チャネルをもつ $1\times 1$ 畳み込みカーネルを用いた相互相関計算を示している。入力と出力は同じ高さと幅をもつことに注意してほしい。出力の各要素は、入力画像の *同じ位置* にある要素の線形結合から導かれる。$1\times 1$ 畳み込み層は、各ピクセル位置に適用される全結合層であり、対応する $c_\textrm{i}$ 個の入力値を $c_\textrm{o}$ 個の出力値へ変換すると考えることができる。とはいえ、依然として畳み込み層なので、重みはピクセル位置をまたいで共有される。したがって、$1\times 1$ 畳み込み層に必要な重みは $c_\textrm{o}\times c_\textrm{i}$ 個（加えてバイアス）である。また、畳み込み層の後には通常、非線形変換が続くことにも注意してほしい。これにより、$1 \times 1$ 畳み込みを他の畳み込みに単純にまとめてしまうことはできなくなる。 
