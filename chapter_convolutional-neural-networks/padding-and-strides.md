...
パディングは出力の高さと幅を増やすことができる。、出力が不必要に縮小するのを避けるために、出力の高さと幅を入力と同じにする目的でよく使われる。さらに、すべての画素が同じ頻度で使われることも保証する。通常は、入力の高さと幅の両側に対称なパディングを選ぶ。この場合、$(p_\textrm{h}, p_\textrm{w})$ パディングと呼ぶ。最も一般的には $p_\textrm{h} = p_\textrm{w}$ とし、その場合は単にパディング $p$ を選ぶと言う。 
