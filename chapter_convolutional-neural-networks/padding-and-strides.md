...
:numref:`fig_correlation` の畳み込みの例を思い出してください。 
入力の高さと幅はともに 3 で、
畳み込みカーネルの高さと幅はともに 2 でした。
その結果、出力表現の次元は $2\times2$ になりました。
入力の形状が $n_\textrm{h}\times n_\textrm{w}$ で、
畳み込みカーネルの形状が $k_\textrm{h}\times k_\textrm{w}$ だとすると、
出力の形状は $(n_\textrm{h}-k_\textrm{h}+1) \times (n_\textrm{w}-k_\textrm{w}+1)$ になります。
つまり、畳み込みを適用できる画素が尽きるまでしか、
畳み込みカーネルを移動できないのです。 

以下では、パディングやストライド付き畳み込みを含むいくつかの手法を見ていきます。
これらは出力サイズをより柔軟に制御するためのものです。
その動機として、カーネルの幅と高さは通常 $1$ より大きいため、
多くの畳み込みを連続して適用すると、
出力は入力よりかなり小さくなりがちであることに注意してください。
...
元画像の境界にある興味深い情報も
すべて失われてしまいます。
*パディング* は、この問題に対処するための最も一般的な手段です。
別の状況では、たとえば元の入力解像度が扱いにくいと感じる場合などに、
次元を大幅に削減したいこともあります。
そのような場合に役立つ一般的な手法が *ストライド付き畳み込み* です。
...
画像の周辺部の画素が失われがちなことです。畳み込みカーネルのサイズと画像内の位置に応じた画素の利用状況を示す :numref:`img_conv_reuse` を考えてみてください。四隅の画素はほとんど使われません。 
...
畳み込み層を何層も重ねるとその損失は積み重なっていきます。
この問題に対する単純な解決策は、
入力画像の境界の周囲に余分な埋め草の画素を追加して、
画像の有効サイズを大きくすることです。
通常、余分な画素の値は 0 に設定します。
:numref:`img_conv_pad` では、$3 \times 3$ の入力にパディングを施して、
サイズを $5 \times 5$ に拡張しています。
それに対応する出力は $4 \times 4$ 行列に増えます。
網掛け部分は最初の出力要素と、その出力計算に使われる入力テンソルおよびカーネルテンソルの要素を表しています: $0\times0+0\times1+0\times2+0\times3=0$。
...
これは、出力の高さと幅がそれぞれ $p_\textrm{h}$ と $p_\textrm{w}$ だけ増えることを意味します。

多くの場合、入力と出力の高さと幅を同じにするために、
$p_\textrm{h}=k_\textrm{h}-1$ および $p_\textrm{w}=k_\textrm{w}-1$ と設定したくなります。
こうすると、ネットワークを構築するときに各層の出力形状を予測しやすくなります。
ここで $k_\textrm{h}$ が奇数だと仮定すると、
高さ方向の両側に $p_\textrm{h}/2$ 行ずつパディングします。
$k_\textrm{h}$ が偶数の場合の1つの方法は、
入力の上側に $\lceil p_\textrm{h}/2\rceil$ 行、
下側に $\lfloor p_\textrm{h}/2\rfloor$ 行をパディングすることです。
幅方向についても同様に両側をパディングします。
...
入力と畳み込みカーネルの相互相関を計算したものだと分かります。

次の例では、高さと幅が 3 の2次元畳み込み層を作成し、
(**すべての辺に 1 ピクセルのパディングを適用します。**)
高さと幅が 8 の入力を与えると、
出力の高さと幅も 8 になることが分かります。
...
その後、下方向と右方向の両方へすべての位置に滑らせていきます。
これまでの例では、1要素ずつ移動するのが既定でした。
しかし、計算効率のためであったり、
ダウンサンプリングしたかったりする場合には、
ウィンドウを1要素より多く移動させて、
途中の位置を飛ばすことがあります。これは、畳み込み
カーネルが大きい場合に特に有用です。なぜなら、基になる画像の広い領域を捉えられるからです。

1回の移動で進む行数と列数を *ストライド* と呼びます。
ここまで、高さ方向と幅方向のストライドはいずれも 1 を使ってきました。
ときには、より大きなストライドを使いたいこともあります。
:numref:`img_conv_stride` は、高さ方向に 3、幅方向に 2 のストライドをもつ
2次元相互相関演算を示しています。
網掛け部分は、出力要素と、その出力計算に使われる入力テンソルおよびカーネルテンソルの要素を表しています: $0\times0+0\times1+1\times2+2\times3=8$, $0\times0+6\times1+0\times2+0\times3=6$。
最初の列の2番目の要素が生成されるとき、
畳み込みウィンドウは3行下に移動していることが分かります。
最初の行の2番目の要素が生成されるとき、
畳み込みウィンドウは2列右に移動しています。
畳み込みウィンドウが入力上をさらに2列右へ移動し続けると、
入力要素がウィンドウを満たせないため出力は得られません
（別の列のパディングを追加しない限り）。
...
出力形状は $(n_\textrm{h}/s_\textrm{h}) \times (n_\textrm{w}/s_\textrm{w})$ になります。
...
パディングは出力の高さと幅を増やすことができます。これは、出力が不必要に縮小するのを避けるために、出力の高さと幅を入力と同じにする目的でよく使われます。さらに、すべての画素が同じ頻度で使われることも保証します。通常は、入力の高さと幅の両側に対称なパディングを選びます。この場合、$(p_\textrm{h}, p_\textrm{w})$ パディングと呼びます。最も一般的には $p_\textrm{h} = p_\textrm{w}$ とし、その場合は単にパディング $p$ を選ぶと言います。 

ストライドにも同様の慣習があります。水平方向のストライド $s_\textrm{h}$ と垂直方向のストライド $s_\textrm{w}$ が一致するとき、単にストライド $s$ と言います。ストライドは出力の解像度を下げることができ、たとえば $n > 1$ のとき、出力の高さと幅を入力の高さと幅の $1/n$ にまで減らすことがあります。既定では、パディングは 0、ストライドは 1 です。 

ここまでで扱ったパディングはすべて、単に画像をゼロで拡張するものでした。これは実現が非常に簡単なので、計算上大きな利点があります。さらに、追加のメモリを割り当てることなく、このパディングを暗黙的に利用するよう演算子を設計できます。同時に、CNN が画像内の暗黙的な位置情報を符号化することも可能にします。つまり、「空白」がどこにあるかを学習するだけでよいのです。ゼロパディング以外にも多くの代替手法があります。:citet:`Alsallakh.Kokhlikyan.Miglani.ea.2020` はそれらについて包括的な概観を与えています（ただし、アーティファクトが生じる場合を除いて、非ゼロパディングをいつ使うべきかについて明確な指針は示していません）。 


## 演習

1. この節の最後のコード例で、カーネルサイズが $(3, 5)$、パディングが $(0, 1)$、ストライドが $(3, 4)$ のとき、  
   出力形状を計算して実験結果と一致するか確認しなさい。
