...
**プーリング（Pooling）とは**、画像認識などの畳み込みニューラルネットワーク（CNN）において、空間的な解像度を下げることで計算量を削減し、位置のわずかなズレに対する不変性を持たせるためのダウンサンプリング処理のことである。代表的な手法に最大プーリングと平均プーリングがある。
