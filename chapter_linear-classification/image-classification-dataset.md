...
データセット :cite:`Xiao.Rasul.Vollgraf.2017` を扱いる。$28 \times 28$ ピクセル解像度の10種類の衣類画像を含んでいる。
...
上の画像はグレースケールで、$32 \times 32$ ピクセル解像度に拡大されている。、元のMNISTデータセットが（2値の）白黒画像で構成されていたのと似ている。ただし、現代の画像データの多くは3チャネル（赤、緑、青）を持ち、ハイパースペクトル画像では100チャネルを超えることもある（HyMapセンサーは126チャネルを持つ）。
