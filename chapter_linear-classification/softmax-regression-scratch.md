...
各行の和は1になる。**]
これは確率として必要な性質である。注意: 上のコードは、非常に大きい値や非常に小さい値に対しては*頑健ではない*。何が起きているかを示すには十分であるが、実用上の目的でこのコードをそのまま使うべきではない。深層学習フレームワークにはこのような保護機構が組み込まれており、今後は組み込みのソフトマックスを使う。
...
1. この節では、ソフトマックス演算の数学的定義に基づいてソフトマックス関数を直接実装した。 :numref:`sec_softmax` で述べたように、これは数値的不安定性を引き起こす可能性がある。
