...
深層学習はこの10年で一種のカンブリア爆発を経験した。
技術、応用、アルゴリズムの数は、過去数十年の進展をはるかに上回っている。
これは複数の要因が幸運にも重なった結果であり、
その一つが、いくつかのオープンソース深層学習フレームワークが提供する強力で無償のツール群である。
...
は、そのような広く採用されたモデルの
第一世代を代表するものと言える。
Lisp 風のプログラミング体験を提供した
SN2 (Simulateur Neuristique) :cite:`Bottou.Le-Cun.1988`
のような初期の先駆的研究とは対照的に、
現代のフレームワークは自動微分と
Python の利便性を備えている。
これらのフレームワークにより、勾配ベースの学習アルゴリズムを実装する際の
...
ニューラルネットワーク層は
きわめて一般的であるため、現代のライブラリはこれらの構成要素も
あらかじめ実装している。
この節では、深層学習フレームワークの
[**高レベル API を用いて**]
...
しかもそれをほぼ毎日行うようになると、
補助となる仕組みのありがたさが分かる。
状況は、ブログシステムをスクラッチから自作するのに似ている。
一度や二度ならやりがいがあり、学びもあるが、
車輪の再発明に1か月費やすようでは、
優れた Web 開発者とは言えない。

標準的な演算については、
[**フレームワークにあらかじめ定義された層を使える**]ため、
実装の細部ではなく、
モデルを構成する層そのものに集中できる。
:numref:`fig_single_neuron` で説明した
単層ネットワークのアーキテクチャを思い出そう。
この層は *全結合* と呼ばれる。
入力の各要素が
行列—ベクトル積によって
各出力に接続されているからである。
...
:begin_tab:`mxnet`
Gluon では、全結合層は `Dense` クラスで定義される。
ここでは単一のスカラー出力だけを生成したいので、
出力数を 1 に設定する。
なお、利便性のため、
Gluon では各層の入力形状を指定する必要がない。
...
:begin_tab:`pytorch`
PyTorch では、全結合層は `Linear` クラスと `LazyLinear` クラス（バージョン 1.8.0 以降で利用可能）で定義される。
後者では、ユーザーは*単に*
出力次元だけを指定すればよいが、
前者ではそれに加えて、
この層に何個の入力が入るかも指定する必要がある。
...
Keras では、全結合層は `Dense` クラスで定義される。
ここでは単一のスカラー出力だけを生成したいので、
出力数を 1 に設定する。
なお、利便性のため、
Keras では各層の入力形状を指定する必要がない。
この線形層に何個の入力が入るかを
Keras に伝える必要はない。
最初にモデルへデータを通すとき、
たとえば後で `net(X)` を実行したときに、
Keras は各層への入力数を自動的に推論する。
この仕組みについては後で詳しく説明する。
:end_tab:
...
`forward` メソッドでは、あらかじめ定義された層の組み込み `__call__` メソッドを呼び出すだけで出力を計算できる。
...
:begin_tab:`mxnet`
`loss` モジュールには、多くの有用な損失関数が定義されている。
速度と利便性のために自前では実装せず、
代わりに組み込みの `loss.L2Loss` を用いる。
これが返す `loss` は
各サンプルごとの二乗誤差なので、
ミニバッチ全体で損失を平均するために `mean` を使う。
:end_tab:

:begin_tab:`pytorch`
[**`MSELoss` クラスは平均二乗誤差を計算する（:eqref:`eq_mse` の $1/2$ 因子は含まない）。**]
デフォルトでは、`MSELoss` はサンプル全体の平均損失を返す。
自前で実装するより高速で、しかも使いやすい。
:end_tab:

:begin_tab:`tensorflow`
`MeanSquaredError` クラスは平均二乗誤差を計算する（:eqref:`eq_mse` の $1/2$ 因子は含まない）。
デフォルトでは、サンプル全体の平均損失を返す。
:end_tab:
...
このアルゴリズムのいくつかの変種をサポートしている。
[**`SGD` インスタンスを生成するときには、**]
最適化対象のパラメータ（モデルの `self.parameters()` から取得できる）と、
...
:numref:`sec_linear_scratch` の `fit_epoch` メソッドの実装に依拠して、
モデルを学習できる。
...
以下では、
[**有限個のデータで学習して得られたモデルパラメータと
真のパラメータを比較する**]。
パラメータにアクセスするには、
対象の層の重みとバイアスを取得すればよい。
スクラッチ実装の場合と同様に、
...
のような現代の深層学習フレームワークがもたらす利便性を活用して
深層ネットワークを実装した。
データの読み込み、層の定義、
損失関数、最適化器、学習ループには
フレームワークのデフォルトを用いた。
...
同時に、これらのモジュールは*自分で実装できる*ことも
忘れてはならない。
これはとりわけ、モデル開発の最前線に立ちたいと願う
意欲的な研究者にとって重要である。
そのような場面では、現在のどのライブラリにも存在しない
新しい構成要素を発明することになるからである。

:begin_tab:`mxnet`
Gluon では、`data` モジュールがデータ処理のためのツールを提供し、
`nn` モジュールが多数のニューラルネットワーク層を定義し、
`loss` モジュールが多くの一般的な損失関数を定義する。
さらに、`initializer` により
多様なパラメータ初期化方法を利用できる。
ユーザーにとって便利なことに、
次元とストレージは自動的に推論される。
...
`nn` モジュールが多数のニューラルネットワーク層と一般的な損失関数を定義する。
末尾が `_` のメソッドで値を置き換えることで、
パラメータを初期化できる。
ネットワークの入力次元を指定する必要があることに注意されたい。
今のところは単純であるが、多数の層を持つ複雑なネットワークを設計するときには、
大きな波及効果を持ちうる。
...
`keras` モジュールが多数のニューラルネットワーク層と一般的な損失関数を定義する。
さらに、`initializers` モジュールが
モデルパラメータのさまざまな初期化方法を提供する。
ネットワークの次元とストレージは自動的に推論される
（ただし、初期化される前にパラメータへアクセスしないよう注意されたい）。
:end_tab:

## 演習

1. ミニバッチ上の損失の総和を使う代わりに、ミニバッチ上の損失の平均を使うようにした場合、学習率はどのように変更する必要があるか？
1. フレームワークのドキュメントを確認して、どの損失関数が提供されているかを見よ。特に、二乗損失を Huber のロバスト損失関数に置き換えよ。すなわち、次の損失関数を使う。
   $$l(y,y') = \begin{cases}|y-y'| -\frac{\sigma}{2} & \textrm{ if } |y-y'| > \sigma \\ \frac{1}{2 \sigma} (y-y')^2 & \textrm{ otherwise}\end{cases}$$
1. モデルの重みの勾配にはどのようにアクセスするか？
1. 学習率とエポック数を変えると、解にどのような影響があるか？改善し続けるだろうか？
1. 生成するデータ量を変えると、解はどのように変わるか？
    1. データ量の関数として、$\\hat{\\mathbf{w}} - \\mathbf{w}$ と $\\hat{b} - b$ の推定誤差をプロットせよ。ヒント: データ量は線形ではなく対数的に増やす。つまり、1000, 2000, ..., 10,000 ではなく、5, 10, 20, 50, ..., 10,000 とする。
    2. なぜこのヒントの提案が適切なのか？