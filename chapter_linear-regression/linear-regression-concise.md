...
`forward` メソッドでは、あらかじめ定義された層の組み込み `__call__` メソッドを呼び出して出力を計算するだけである。
...
モデルパラメータの初期化方法をさまざまに提供する。
ネットワークの次元とストレージは自動的に推論される
（ただし、初期化される前にパラメータへアクセスしようとしないよう注意されたい）。
:end_tab:
...
1. 学習率とエポック数を変えると、解にどのような影響があるか？改善し続けますか？
