...
、未知のデータに対して良い性能を発揮してほしいと考える。
古典的な汎化理論は、訓練性能とテスト性能の差を縮めるには、
単純なモデルを目指すべきだと示唆している。
単純さは、次元数が少ないという形で現れる。
、 :numref:`sec_generalization_basics` で線形モデルの単項式基底関数を議論したときに見た。
...
、 :numref:`sec_linear_scratch` で定義した `fit_epoch` メソッドであるでに処理済みである。
:end_tab:
