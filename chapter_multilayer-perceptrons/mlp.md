...
高水準 API を用いて実装した（:numref:`sec_softmax_concise`）。
これにより、低解像度画像から 10 種類の衣類を認識できる分類器を学習できた。
その過程で、データを整形する方法、
出力を妥当な確率分布に変換する方法、
適切な損失関数を適用する方法、
そしてモデルのパラメータに関してそれを最小化する方法を学んだ。
単純な線形モデルの文脈でこれらの仕組みを理解したところで、いよいよ深層ニューラルネットワークの探究に進むことにしよう。
本書が主として扱うのは、より豊かな表現力を持つこれらのモデルである。
...
その上で線形モデルが適切になる可能性はあるが、
それを手作業で計算する方法は分かりない。
深層ニューラルネットワークでは、観測データを用いて、
隠れ層による表現と、その表現に作用する線形予測器の両方を
同時に学習した。

この非線形性の問題は、少なくとも 1 世紀にわたって研究されてきた :cite:`Fisher.1928`。
たとえば、決定木は最も基本的な形では、
一連の二値決定を用いてクラス所属を判定する :cite:`quinlan2014c4`。
同様に、カーネル法は非線形依存をモデル化するために
何十年も使われてきた :cite:`Aronszajn.1950`。
非パラメトリックなスプラインモデル :cite:`Wahba.1990` や
カーネル法 :cite:`Scholkopf.Smola.2002` にもつながっている。
また、脳はこれをごく自然に解いている。
結局のところ、ニューロンは他のニューロンへ入力し、
そのニューロンはさらに別のニューロンへ入力する :cite:`Cajal.Azoulay.1894`。
...
では、その代わりに何を得たのだろうか？
驚くかもしれないが、上で定義したモデルでは、*苦労のわりに何も得ていない*！
理由は明白である。
...
再生核ヒルベルト空間の文脈では :citet:`micchelli1984interpolation` があり、
 1 つの隠れ層を持つ RBF（radial basis function）ネットワークと見なせる形である。
これらの結果（および関連する結果）は、
...
最も人気のある選択肢は *rectified linear unit*（*ReLU*）である :cite:`Nair.Hinton.2010`。
[**ReLU は非常に単純な非線形変換を提供する**]。
...
、入力が実際には 0 にならないかもしれないので問題ありない
（数学者は、測度 0 の集合上で非微分可能だと言うだろう）。
微妙な境界条件が重要なら、
おそらく工学ではなく（本当の）数学をしているのだ、という古い格言がある。
ここでもその常識が当てはまるかもしれないし、
少なくとも制約付き最適化を行っていないという事実が関係しているのかもしれない :cite:`Mangasarian.1965,Rockafellar.1970`。
...
シグモイドはソフトマックスの特殊な場合と考えることができる。
しかし、隠れ層での多くの用途では、
シグモイドはより単純で学習しやすい ReLU にほぼ置き換えられた。
その大きな理由は、
シグモイドが最適化において難しさをもたらすからである :cite:`LeCun.Bottou.Orr.ea.1998`。
