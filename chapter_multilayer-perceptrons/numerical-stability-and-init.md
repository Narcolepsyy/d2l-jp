...
あらかじめ定めた分布に従って
パラメータを初期化する必要があった。
これまでは初期化手法を所与のものとして扱い、
その選択方法の詳細には立ち入らなかった。
そのため、こうした選択はそれほど重要ではない
という印象を持ったかもしれない。
しかし実際には、初期化手法の選択は
ニューラルネットワークの学習において重要な役割を果たし、
数値安定性を保つうえで決定的になることもある。
さらに、これらの選択は
非線形活性化関数の選択とも興味深い形で結びついている。
どの関数を選ぶか、そしてパラメータをどう初期化するかによって、
最適化アルゴリズムがどれだけ速く収束するかが左右される。
ここでの選択を誤ると、学習中に
勾配爆発や勾配消失に直面することがある。
...
$L$ 層の深いネットワークを考える。
各層 $l$ は、重み $\mathbf{W}^{(l)}$ によってパラメータ化された変換 $f_l$
で定義され、その隠れ層出力を $\mathbf{h}^{(l)}$ とする（$\mathbf{h}^{(0)} = \mathbf{x}$ とする）。
...
$\mathbf{o}$ の任意のパラメータ集合 $\mathbf{W}^{(l)}$ に関する勾配は次のように書ける。
...
と勾配ベクトル $\mathbf{v}^{(l)}$ の積である。
したがって、
あまりに多くの確率を掛け合わせたときにしばしば生じる
数値アンダーフローと同様の影響を受ける。
確率を扱うときの一般的な工夫は、
対数空間に移ること、すなわち
数値表現の仮数部から指数部へと
負担を移すことである。
残念ながら、ここでの問題はそれより深刻である。
初期状態では、行列 $\mathbf{M}^{(l)}$ はさまざまな固有値を持ちうる。
それらは小さいことも大きいこともあり、
その積は *非常に大きく* も *非常に小さく* もなりうる。
...
勾配消失問題のよくある原因の一つは、
各層の線形演算の後に適用する活性化関数 $\sigma$
の選択である。
歴史的には、シグモイド関数
$1/(1 + \exp(-x))$（:numref:`sec_mlp` で導入）
は、しきい値関数に似ているため広く用いられていた。
初期の人工ニューラルネットワークは
生物学的ニューラルネットワークに着想を得ていたため、
生体ニューロンのように *完全に* 発火するか *まったく* 発火しないかの
どちらかであるニューロンという考え方は魅力的に見えた。
シグモイドを詳しく見て、
なぜ勾配消失を引き起こしうるのかを確認しよう。
...
図から分かるように、(**シグモイドの勾配は、入力が大きいときも小さいときも消失する**）。
さらに、多くの層を逆伝播するとき、
多くのシグモイドへの入力がゼロ近傍の
勾配が十分に得られる範囲にない限り、
全体の勾配の積は消失してしまう可能性がある。
ネットワークが深くなるほど、
注意しなければ、どこかの層で勾配が
途切れてしまう。
実際、この問題はかつて深層ネットワークの学習を大きく妨げていた。
その結果、より安定した
（ただし生物学的にはあまりもっともらしくない）
ReLU が、実務上の標準的な選択肢として
広く使われるようになった。
...
（分散 $\sigma^2=1$）では、
行列積は爆発する。
これが深いネットワークの初期化で起これば、
勾配降下法に基づく最適化器が収束する見込みはない。
...
ニューラルネットワーク設計における
もう一つの問題は、パラメータ化に内在する対称性である。
1 つの隠れ層と 2 つのユニットを持つ
単純な MLP を考える。
この場合、最初の層の重み $\mathbf{W}^{(1)}$
を入れ替え、同様に出力層の重みも入れ替えれば、
同じ関数を得られる。
...
これは単なる理論上の厄介ごとではない。
先ほどの 1 隠れ層・2 ユニットの MLP を考えよう。
説明のために、
出力層が 2 つの隠れユニットを 1 つの出力ユニットに写像するとする。
もし隠れ層のすべてのパラメータを
...
したがって、勾配ベースの反復法（たとえばミニバッチ確率的勾配降下法）を行っても、
$\mathbf{W}^{(1)}$ のすべての要素は
依然として同じ値のままである。
このような反復だけでは
自力で対称性を *破る* ことは決してできず、
ネットワークの表現力を
十分に引き出せないままになるかもしれない。
隠れ層は、あたかも
1 つのユニットしか持たないかのように振る舞うだろう。
...
重み $w_{ij}$ はすべて
同じ分布から独立にサンプルされるとする。
さらに、この分布の平均が 0、分散が $\sigma^2$ であると仮定する。
ここで、分布がガウス分布である必要はなく、
平均と分散が存在すれば十分であることに注意されたい。
今のところ、この層への入力 $x_j$ も
平均 0、分散 $\gamma^2$ を持ち、
$w_{ij}$ と互いに独立で、かつ入力同士も独立であると仮定しよう。
この場合、$o_i$ の平均は次のように計算できる。
...
分散を一定に保つ一つの方法は、
$n_\textrm{in} \sigma^2 = 1$ とすることである。
次に逆伝播を考える。
ここでも同様の問題に直面するが、
今度は勾配が出力に近い層から伝播してくる。
順伝播の場合と同じ考え方を使うと、
$n_\textrm{out} \sigma^2 = 1$
でない限り、勾配の分散は爆発しうることが分かる。
ここで $n_\textrm{out}$ はこの層の出力数である。
すると、ジレンマに陥る。
この 2 つの条件を同時に満たすことはできない。
そこで、次を満たすことを目指す。
...
これが、現在では標準的で実用上も有益な
*Xavier 初期化* の理論的根拠である。
この手法は、その考案者の第一著者にちなんで名付けられた :cite:`Glorot.Bengio.2010`。
通常、Xavier 初期化では
平均 0、分散
...
分散の選び方にも適用できる。
一様分布 $U(-a, a)$ の分散は $\frac{a^2}{3}$ であることに注意されたい。
...
上の数学的な議論では
非線形性がないことを仮定しているが、
この仮定はニューラルネットワークでは容易に破られる。
それでも、Xavier 初期化は
実際にはうまく機能することが分かっている。
...
アーキテクチャ上の工夫なしに
10,000 層のニューラルネットワークを学習できる可能性を示した。

この話題に興味があるなら、
このモジュールで扱う各手法を深く掘り下げ、
それぞれの経験則を提案・解析した論文を読み、
さらにこの分野の最新論文を追ってみるとよい。
巧妙なアイデアを見つけたり、
あるいは自ら考案したりして、
深層学習フレームワークへの実装に貢献することもあるかもしれない。


## まとめ

勾配消失と勾配爆発は、深いネットワークでよく見られる問題である。勾配とパラメータを適切な範囲に保つためには、パラメータ初期化に細心の注意が必要である。
初期勾配が大きすぎず小さすぎもしないようにするには、初期化に関する経験則が必要である。
ランダム初期化は、最適化の前に対称性を破ることを保証するうえで重要である。
Xavier 初期化は、各層において、任意の出力の分散が入力数の影響を受けず、任意の勾配の分散が出力数の影響を受けないようにすることを意図している。
ReLU 活性化関数は勾配消失問題を緩和し、収束を加速する。

## 演習

1. MLP の各層における置換対称性以外に、対称性を破る必要があるニューラルネットワークの例を設計できるか？
1. 線形回帰や softmax 回帰では、すべての重みパラメータを同じ値に初期化してもよいだろうか？
1. 2 つの行列の積の固有値に関する解析的な上界を調べよ。これは、勾配が良条件となるように保つことについて何を示唆しているだろうか？
1. ある項が発散すると分かっている場合、後から修正できるだろうか？ layerwise adaptive rate scaling に関する論文を参照されたい :cite:`You.Gitman.Ginsburg.2017`。