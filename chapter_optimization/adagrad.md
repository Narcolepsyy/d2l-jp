...
言語モデルを学習していると想像してほしい。十分な精度を得るには、通常、学習を続けるにつれて学習率を下げていく必要があり、その減衰率はたいてい $\mathcal{O}(t^{-\frac{1}{2}})$ かそれより遅い程度である。ここで、疎な特徴、すなわちごくまれにしか現れない特徴をもつモデルの学習を考える。自然言語ではよくあることで、たとえば *learning* よりも *preconditioning* という単語に出会う確率のほうがずっと低いだろう。しかし、計算広告や個人化された協調フィルタリングなど、他の分野でも同様に一般的である。結局のところ、少数の人にしか関心を持たれないものはたくさんある。
...
ここでは $\bar{\mathbf{x}} = \mathbf{U} \mathbf{x}$、したがって $\bar{\mathbf{c}} = \mathbf{U} \mathbf{c}$ を用いた。変換後の問題の最小解は $\bar{\mathbf{x}} = -\boldsymbol{\Lambda}^{-1} \bar{\mathbf{c}}$、最小値は $-\frac{1}{2} \bar{\mathbf{c}}^\top \boldsymbol{\Lambda}^{-1} \bar{\mathbf{c}} + b$ である。$\boldsymbol{\Lambda}$ は $\mathbf{Q}$ の固有値を並べた対角行列なので、ずっと計算しやすくなる。
...
条件数 $\kappa$ が大きいと、最適化問題を正確に解くのは困難である。値のダイナミックレンジが大きい場合にも正しく扱えるよう、注意深くする必要がある。この分析から、素朴ではあるものの明らかな疑問が生じる。すべての固有値が $1$ になるように空間を歪めてしまえば、問題を単純に「修正」できないのだろうか。理論上はかなり簡単である。$\mathbf{Q}$ の固有値と固有ベクトルさえあれば、問題を $\mathbf{x}$ から $\mathbf{z} \stackrel{\textrm{def}}{=} \boldsymbol{\Lambda}^{\frac{1}{2}} \mathbf{U} \mathbf{x}$ へと再スケーリングできる。新しい座標系では $\mathbf{x}^\top \mathbf{Q} \mathbf{x}$ は $\|\mathbf{z}\|^2$ に簡約できる。とはいえ、かなり実用的でない提案である。一般に、固有値と固有ベクトルを計算することは、実際の問題を解くことよりも *はるかに* 高コストだからである。

固有値を正確に計算するのは高価かもしれないが、それを推測し、多少なりとも近似的に計算するだけでも、何もしないよりはずっとよい場合がある。特に、$\mathbf{Q}$ の対角成分を使って、それに応じて再スケーリングできる。固有値を計算するよりも *はるかに* 安価である。
...
$\mathbf{s}_t$ に二乗勾配を蓄積するということは、$\mathbf{s}_t$ が本質的に線形の速度で増加することを意味する（実際には、勾配が最初は減少するので、ややそれより遅い）。これにより、座標ごとに調整された $\mathcal{O}(t^{-\frac{1}{2}})$ の学習率が得られる。凸問題に対しては、まったく十分である。しかし深層学習では、学習率をもう少しゆっくり下げたい場合がある。そのため、後続の章で議論するいくつかの Adagrad 変種が生まれた。ここではまず、二次の凸問題でどのように振る舞うかを見てみよう。先ほどと同じ問題を使う。
...
学習率を $2$ に増やすと、よりよい挙動が見られる。、ノイズのない場合であっても学習率の減少がかなり急激になりうることを示しており、パラメータが適切に収束するように注意する必要があることがわかる。
...
1. 直交行列 $\mathbf{U}$ とベクトル $\mathbf{c}$ に対して、次が成り立つことを証明せよ: $\|\mathbf{c} - \mathbf{\delta}\|_2 = \|\mathbf{U} \mathbf{c} - \mathbf{U} \mathbf{\delta}\|_2$. なぜ、直交変数変換の後でも摂動の大きさが変わらないことを意味するのか。
