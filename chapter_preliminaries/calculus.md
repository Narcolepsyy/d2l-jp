...
*微分学* と *積分学* の両方の根底にある。
前者は、引数を操作することで
関数値をどのように増減させるかを教えてくれる。
、損失関数を減らすために
パラメータを繰り返し更新する深層学習で直面する
*最適化問題* において非常に役立ちる。
最適化はモデルを訓練データに適合させるプロセスであり、微分積分学はそのための重要な基礎知識である。
ただし、最終的な目標は
*これまで見たことのない* データで良い性能を発揮することだという点を忘れてはいけない。
この問題は *汎化* と呼ばれ、
他の章で重要な焦点となる。
...
簡単に言えば、*導関数* とは、引数の変化に対する関数の変化率である。
導関数は、各パラメータを
微小な量だけ *増減* させたときに、
損失関数がどれだけ変化するかを示してくれる。
...
導関数には、いくつか同値な記法がある。
$y = f(x)$ とすると、次の表現はすべて同値である。

$$f'(x) = y' = \frac{dy}{dx} = \frac{df}{dx} = \frac{d}{dx} f(x) = Df(x) = D_x f(x),$$

ここで、$\frac{d}{dx}$ と $D$ は *微分作用素* である。
以下に、いくつかの一般的な関数の導関数を示す。
...
これを用いると、次のようにして $3 x^2 - 4x$ の導関数を求められる。

$$\frac{d}{dx} [3 x^2 - 4x] = 3 \frac{d}{dx} x^2 - 4 \frac{d}{dx} x = 6x - 4.$$

$x = 1$ を代入すると、確かにこの位置での導関数は $2$ になる。
導関数は、ある特定の位置における関数の *傾き* を教えてくれることに注意しよう。  
...
より鮮明な画像のために SVG 形式でグラフを出力するよう指示す。
コメント `#@save` は特別な修飾子で、
...
便利なことに、`set_figsize` で図のサイズを設定できる。
...
ここでのコードの多くは、入力のサイズと形状が一致することを確認しているだけである。
...
ここまでは、1 変数だけの関数を微分してきた。
深層学習では、*多変数* の関数も扱う必要がある。
そのような *多変数* 関数に適用される導関数の概念を簡単に導入する。


$y = f(x_1, x_2, \ldots, x_n)$ を $n$ 個の変数をもつ関数とする。
...
$\nabla f(\mathbf{x})$ と書かれる。
多変数関数を微分する際には、次の規則が役立ちる。

* すべての $\mathbf{A} \in \mathbb{R}^{m \times n}$ について、$\nabla_{\mathbf{x}} \mathbf{A} \mathbf{x} = \mathbf{A}^\top$ および $\nabla_{\mathbf{x}} \mathbf{x}^\top \mathbf{A}  = \mathbf{A}$ が成り立ちる。
...
$\nabla_{\mathbf{X}} \|\mathbf{X} \|_\textrm{F}^2 = 2\mathbf{X}$ が成り立ちる。 



## 連鎖律

深層学習では、扱う勾配はしばしば計算が難しくなる。
なぜなら、
深く入れ子になった関数
（関数の中の関数の中の関数…）を扱っているからである。
幸いなことに、*連鎖律* がこれを処理してくれる。
1 変数関数に戻ると、
$y = f(g(x))$ であり、基礎となる関数
$y=f(u)$ と $u=g(x)$ が
どちらも微分可能だとする。
...
ベクトル $\mathbf{u}$ のベクトル $\mathbf{x}$ に関する導関数を含む *行列* である。
したがって、勾配を評価するには
ベクトルと行列の積を計算する必要がある。
...
1. ここまでは導関数の規則を当然のものとして扱ってきた。 
   定義と極限を用いて、(i) $f(x) = c$, (ii) $f(x) = x^n$, (iii) $f(x) = e^x$, (iv) $f(x) = \log x$ の性質を証明しよう。
1. 同様に、積の法則、和の法則、商の法則を第一原理から証明しよう。 
1. 定数倍の法則が積の法則の特殊な場合として導かれることを証明しよう。 
1. $f(x) = x^x$ の導関数を求めよ。 
1. ある $x$ に対して $f'(x) = 0$ であるとはどういう意味だろうか。 
   そのようなことが成り立つ関数 $f$ と位置 $x$ の例を挙げよ。 
1. 関数 $y = f(x) = x^3 - \frac{1}{x}$ を描画し、$x = 1$ における接線も描画しよう。
1. 関数 
