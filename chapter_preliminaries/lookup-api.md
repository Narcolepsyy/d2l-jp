...
# ドキュメント
:begin_tab:`mxnet`
すべての MXNet の関数やクラスをここで紹介することは到底できないし、
それらの情報はすぐに古くなってしまう可能性もある。
...
最新かつ詳細な情報を提供している。
この節では、MXNet の API を効率的に調べるためのヒントを紹介する。
:end_tab:

:begin_tab:`pytorch`
すべての PyTorch の関数やクラスをここで紹介することは到底できないし、
その情報はすぐに古くなってしまうかもしれないが、
...
すべての TensorFlow の関数やクラスをここで紹介することは到底できないし、
その情報はすぐに古くなってしまうかもしれないが、
...
`dir` 関数を使う。たとえば、
[**乱数生成のためのモジュール内のすべての属性を問い合わせる**]ことができる。
...
単一の `_` で始まる関数（通常は内部関数）は無視して構わない。
残った関数名や属性名から判断すると、
...
私たちは、単なる網羅的なリストを作るよりも、実際に問題を解決するために役立つ重要な使用例を重視して紹介していきる。
また、ライブラリのソースコードを調べて、
本番コードの高品質な実装例を見ることも勧める。
