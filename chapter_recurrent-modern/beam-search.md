# ビームサーチ（Beam Search）とは：生成モデルの推論アルゴリズム
:label:`sec_beam-search`

**ビームサーチ（Beam Search）とは**、Seq2Seqなどの生成モデルにおいて、各ステップで上位K個の尤もらしい候補（ビーム幅）を保持しながら探索を行うことで、貪欲法（Greedy Search）よりも精度の高い出力系列を得るためのヒューリスティックな探索アルゴリズムである。
...
1. :numref:`sec_rnn-scratch` では、ユーザが与えたプレフィックスに続くテキストを生成するために言語モデルを使った。どの種類の探索戦略を使っているか。改善できるか。
