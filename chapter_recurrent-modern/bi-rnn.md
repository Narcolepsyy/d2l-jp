...
同じ入力に対して、逆方向に連結された2つの一方向 RNN 層を実装するだけである（:numref:`fig_birnn`）。
...
双方向 RNN をゼロから実装したい場合は、別々の学習可能パラメータを持つ2つの一方向 `RNNScratch` インスタンスを含めればよい。
...
双方向 RNN では、各時刻の隠れ状態は、現在の時刻より前と後のデータの両方によって同時に決まる。双方向 RNN は主に、系列の符号化や、双方向の文脈が与えられた観測の推定に有用である。双方向 RNN は、長い勾配連鎖のために学習コストが非常に高くなる。
