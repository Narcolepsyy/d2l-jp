...
機械翻訳ではここでは単語単位のトークン化を用いるのが一般的である
...
[**固定長であった**]
ことを思い出されたい。
 :numref:`sec_language-model` の `num_steps`
...
1. `_tokenize` メソッドの `max_examples` 引数にさまざまな値を試してみよ。ソース言語とターゲット言語の語彙サイズにどのような影響を与えるか。
