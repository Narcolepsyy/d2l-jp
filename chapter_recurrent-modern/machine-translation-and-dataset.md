...
この節では、機械翻訳問題と、
後続の例で用いるデータセットの例を紹介します。
何十年もの間、言語間翻訳の統計的定式化は
研究者がニューラルネットワークによる手法を実用化する以前から
人気がありました :cite:`Brown.Cocke.Della-Pietra.ea.1988,Brown.Cocke.Della-Pietra.ea.1990`
（これらの手法はしばしばまとめて *neural machine translation* という用語で呼ばれていました）。


まず、データを処理するための新しいコードが必要です。
:numref:`sec_language-model` で見た言語モデリングとは異なり、
ここでは各例が2つの別々のテキスト系列からなり、
一方はソース言語、もう一方は（翻訳された）ターゲット言語です。
以下のコード片では、前処理済みデータを
学習用のミニバッチに読み込む方法を示します。
...
データセットの各行はタブ区切りのペアで、
英語のテキスト系列（*source*）と
翻訳されたフランス語のテキスト系列（*target*）から構成されます。
なお、各テキスト系列は
1文だけの場合もあれば、
複数文からなる段落の場合もあります。
...
（現在の最先端モデルでは、より複雑なトークン化手法が使われています）。
以下の `_tokenize` メソッドは、
最初の `max_examples` 個のテキスト系列ペアをトークン化します。
各トークンは単語または句読点のいずれかです。
各系列の末尾には、系列の終端を示す特別な “&lt;eos&gt;” トークンを追加します。
モデルがトークンを1つずつ生成しながら系列を予測する場合、
“&lt;eos&gt;” トークンの生成は出力系列が完了したことを示唆します。
最終的に、以下のメソッドは `src` と `tgt` という
2つのトークン列のリストを返します。
具体的には、`src[i]` はソース言語（ここでは英語）の
$i^\textrm{th}$ テキスト系列のトークン列であり、
`tgt[i]` はターゲット言語（ここではフランス語）のトークン列です。
...
テキスト系列ごとのトークン数のヒストグラムを[**描いてみましょう。**]
この単純な英仏データセットでは、
ほとんどのテキスト系列は20トークン未満です。
...
機械翻訳では、各例は
ソース系列とターゲット系列のペアであり、
2つのテキスト系列の長さは異なっていてもかまいません。

計算効率のため、
*切り詰め* と *パディング* によって、
テキスト系列のミニバッチを一度に処理することができます。
同じミニバッチ内のすべての系列が
同じ長さ `num_steps` を持つと仮定します。
あるテキスト系列のトークン数が `num_steps` より少ない場合は、
その末尾に特別な "&lt;pad&gt;" トークンを
長さが `num_steps` に達するまで追加し続けます。
そうでない場合は、
...
同じ形状のミニバッチに読み込めるよう、
同じ長さになります。
さらに、パディングトークンを除いたソース系列の長さも記録します。
この情報は、後で扱ういくつかのモデルで必要になります。
...
これを緩和するために、
ここでは2回未満しか現れない
低頻度トークンをすべて同じ未知（"&lt;unk&gt;"）トークンとして扱います。
...
ターゲット系列を予測する際の最初の入力トークンとして使われます
...
最後に、データ反復子を返す `get_dataloader` メソッドを定義します。
...
英仏データセットから最初のミニバッチを[**読み込んでみましょう。**]
...
自動的に写像する課題を指します。
単語単位のトークン化を用いると、
語彙サイズは文字単位のトークン化よりもかなり大きくなりますが、
系列長ははるかに短くなります。
大きな語彙サイズを緩和するために、
低頻度トークンを何らかの「未知」トークンとして扱うことができます。
テキスト系列は切り詰めとパディングによって同じ長さにそろえ、
同じ形状のミニバッチとして読み込めるようにします。
現代の実装では、パディングによる無駄な計算を避けるために、
長さの近い系列をバケット化することがよくあります。 
