...
# シーケンス・ツー・シーケンス（Seq2Seq）とは：機械翻訳の実践
:label:`sec_seq2seq`

**シーケンス・ツー・シーケンス（Seq2Seq）とは**、入力系列（ソース）を固定長のベクトルに変換するエンコーダと、そのベクトルから出力系列（ターゲット）を生成するデコーダを組み合わせ、機械翻訳や要約などのタスクを実現する深層学習の枠組みである。
...
(:numref:`sec_encoder-decoder`) に依拠する。
この節では、
...
第2に、エンコーダの最終隠れ状態を
デコーダへ
各デコード時刻ごとに入力することもできる :cite:`Cho.Van-Merrienboer.Gulcehre.ea.2014`。
...
デコーダの入力と出力の扱いには
より注意が必要である。
最も一般的な方法は、しばしば *teacher forcing* と呼ばれる。
...
英仏データセット上で機械翻訳のために学習する。

## エンコーダ

エンコーダは、長さ可変の入力系列を
固定形状の *コンテキスト変数* $\mathbf{c}$ に変換することを思い出されたい
(:numref:`fig_seq2seq` を参照)。


単一の系列例（バッチサイズ1）を考える。
入力系列が $x_1, \ldots, x_T$ であり、
$x_t$ が $t^{\textrm{th}}$ トークンだとする。
時刻 $t$ において、RNNは
$x_t$ に対応する入力特徴ベクトル $\mathbf{x}_t$ と
前時刻の隠れ状態 $\mathbf{h} _{t-1}$ を
現在の隠れ状態 $\mathbf{h}_t$ に変換する。
RNNの再帰層の変換は、
関数 $f$ を用いて次のように表せる。
...
具体例を使って、
上のエンコーダ実装を[**説明**]しよう。
以下では、隠れユニット数が16の2層GRUエンコーダを生成する。
系列入力 `X` のミニバッチ
...
（入力系列の時刻と区別するために $t^\prime$ を用いる）が与えられると、
デコーダは、ターゲット側の前のトークン
$y_1, \ldots, y_{t'}$ と
コンテキスト変数
$\mathbf{c}$ に条件づけて、
ステップ $y_{t'+1}$ に現れうる各トークンに対する予測確率を割り当てる。
すなわち、$P(y_{t'+1} \mid y_1, \ldots, y_{t'}, \mathbf{c})$ である。
...
以下のようにデコーダを実装する際には、
エンコーダの最終時刻の隠れ状態を
そのままデコーダの隠れ状態の初期化に用いる。
このため、RNNエンコーダとRNNデコーダは
同じ層数と同じ隠れユニット数を持つ必要がある。
...
:numref:`fig_seq2seq_details` にまとめられている。
...
最も高い確率を持つトークンを選ぶことである。
学習時と同様に、初期時刻には
系列開始 ("&lt;bos&gt;") トークンを
デコーダに入力する。
この予測過程は
:numref:`fig_seq2seq_predict` に示されている。
...
次の節では、
ビームサーチに基づく
より洗練された戦略を導入する (:numref:`sec_beam-search`)。
...
予測系列が $A$, $B$ だとすると、
$p_1 = p_2 = 1$ であっても、
ペナルティ係数 $\exp(1-6/2) \approx 0.14$ によりBLEUは低くなる。
...
2つのRNNを用いてシーケンス・ツー・シーケンス学習のモデルを設計できる。
エンコーダ--デコーダの学習では、teacher forcing により
予測ではなく元の出力系列をデコーダに入力する。
エンコーダとデコーダの実装には、
多層RNNを使える。
...
$n$-gram を照合する代表的な尺度である。


## 演習

1. 翻訳結果を改善するためにハイパーパラメータを調整できるか？
1. 損失計算でマスクを使わずに実験を再実行せよ。どのような結果が観察されるか。なぜだろうか。
1. エンコーダとデコーダの層数または隠れユニット数が異なる場合、デコーダの隠れ状態をどのように初期化できるか？
1. 学習時に、teacher forcing の代わりに前時刻の予測をデコーダへ入力するように置き換えよ。性能にどのような影響があるか。
1. GRUをLSTMに置き換えて実験を再実行せよ。
1. デコーダの出力層を設計する他の方法はあるか？
