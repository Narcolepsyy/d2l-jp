...
各コンポーネントがどのように動作するかを理解できるように設計されていた。
しかし、日常的にRNNを使うときや
本番コードを書くときには、
実装時間
...
以下のクラスを、
高水準APIで実装されたRNNを用いて定義する。

:begin_tab:`mxnet`
具体的には、隠れ状態を初期化するために、
メンバーメソッド `begin_state` を呼び出す。
これは、ミニバッチ内の各サンプルに対する初期隠れ状態を含むリストを返す。
その形状は
（隠れ層数、バッチサイズ、隠れユニット数）
である。
後で導入するいくつかのモデル
...
:begin_tab:`jax`
現時点では、Flaxは通常のRNNを簡潔に実装するためのRNNCellを提供していない。
...
次の `RNNLM` クラスはRNNベースの完全な言語モデルを定義する。
...
1. :numref:`sec_sequence` の自己回帰モデルをRNNを用いて実装せよ。
